 ...
 * @param npmPackageName NPM package representing the given repository. Angular repositories
 *   usually contain multiple packages in a monorepo scheme, but packages commonly are released
 *   with the same versions. This means that a single package can be used for querying NPM about
 *   previously published versions (e.g. to determine active LTS versions). The package name is
 *   used to check if the given branch is containing an active LTS version.
 * @param branchName Branch that is checked to be an active LTS version-branch.
 * */
export async function assertActiveLtsBranch(
    repo: GithubRepo, npmPackageName: string, branchName: string) {
  const version = await getVersionOfBranch(repo, branchName);
  const {'dist-tags': distTags, time} =
      await (await fetch(`https://registry.npmjs.org/${npmPackageName}`)).json();
